...
void histo_replay(Trace* trace){
	...
	int comm_id, datatype_id, datatype_size, count, dest, tag, ret = MPI_ERR_OTHER, indexfh, offset, flags, fdindex, fd;
	void *sendbuf = 0;
	int i = 0;
	char ch[256], openmode[10];
	char *chp;
	FILE *fp;
	extern map<int, Fileopen> file_open;
	map<int, Fileopen>::iterator file_it;
	...
	while(myContext->hasNext()){
		...
		if( i % 100 == 0 && my_rank == doTestFor )
			printMsgCounts();
#endif
		struct timeval begin, end, begin1, end1, iobegin, ioend;
		long long whole, frac, iotime, ioleft=0, iosleep;
		gettimeofday(&begin1, NULL);

		/*
		if(my_rank == 0)
			cout <<"op_code: "<<op_code<<" eventid="<<e->getId()<<" rank="<<my_rank<<"\n";
			*/
		...
		switch(op_code){
			...
			case umpi_MPI_File_open:
				replay_File_open(e, replay_timer);
				indexfh = e->getCurrentValue(FH);
				file_it = file_open.find(indexfh);
				assert(file_it != file_open.end());
				replay_timer->simulateComputeTime();
				ret = MPI_File_open((file_it->second).comm, (file_it->second).ch, (file_it->second).amode, (file_it->second).info, &fh);
				replay_timer->setTime();
#ifdef IOTIMER
				iotime = e->getCurrentValue(TIME);
				gettimeofday(&ioend, NULL);
				iosleep = (ioend.tv_sec-iobegin.tv_sec)*1000000+(ioend.tv_usec-iobegin.tv_usec);
				if((iosleep = iotime-iosleep+ioleft) > 0){
					usleep(iosleep);
					ioleft = 0;
				}else{
					ioleft = iosleep-iotime+ioleft;
				}
				gettimeofday(&iobegin, NULL);
#endif
				(file_it->second).fh = fh;
				if(ret != MPI_SUCCESS){
					cerr << "error: MPI_File_open did not return MPI_SUCCESS"<<endl;
					exit(0);
				}
				break;
			case umpi_MPI_File_close:
				indexfh = e->getCurrentValue(FH);
				file_it = file_open.find(indexfh);
				if(file_it != file_open.end()){
					fh = (file_it->second).fh;
					replay_timer->simulateComputeTime();
			...
			case umpi_fopen:
				...
				ch[i] = '\0';
				if(op_code == umpi_open){
					flags = e->getCurrentValue(POSIX_FLAGS);
					if(access(ch, 0) == 0){
						replay_timer->simulateComputeTime();
						fd = open(ch, flags);
						replay_timer->setTime();
						...
						replay_timer->simulateComputeTime();
						fd = open(ch, flags, S_IREAD|S_IWRITE);
						replay_timer->setTime();
					}
					add_fds_handle_entry(replay_ptr, fd);
					if(fd <=0 ){
						cerr << "error: open did not return success"<<endl;
						exit(0);
					}
				}
				else if(op_code == umpi_open64){
					flags = e->getCurrentValue(POSIX_FLAGS);
					if(access(ch, 0) == 0){
						replay_timer->simulateComputeTime();
						fd = open64(ch, flags);
						...
						replay_timer->simulateComputeTime();
						fd = open64(ch, flags, S_IREAD|S_IWRITE);
						replay_timer->setTime();
					}
					add_fds_handle_entry(replay_ptr, fd);
					if(fd <=0 ){
				...
				else if(op_code == umpi_fopen){
					i = 0;
					chp = replay_open(e, POSIX_MODE);
					while(*chp != '\0'){
						openmode[i] = *chp;
						i += 1;
						chp += 1;
					...
					openmode[i] = '\0';
					replay_timer->simulateComputeTime();
					fp = fopen(ch, openmode);
					replay_timer->setTime();
					add_ffiles_handle_entry(replay_ptr, fp);
					if(fp == NULL){
						cerr << "error: fopen did not return success"<<endl;
						exit(0);
					}
				}
#ifdef IOTIMER
				...
				break;
			case umpi_fclose:
				fdindex = e ->getParamValueForRank(POSIX_FD, my_rank);
				fp = index_to_ffiles_handle(replay_ptr, fdindex);
				if(fp == NULL){
			...
			case umpi_MPI_File_iwrite_at:
				...
				indexfh = e->getCurrentValue(FH);
				if(op_code == umpi_MPI_File_iread_at | op_code == umpi_MPI_File_iwrite_at)
		...
		if(my_rank == 0){
		  if(end1.tv_usec < begin1.tv_usec){
		    whole = end1.tv_sec - begin1.tv_sec - 1;
		    frac = end1.tv_usec + 1000000 - begin1.tv_usec;
		  } else {
		    whole = end1.tv_sec - begin1.tv_sec;
		    frac = end1.tv_usec - begin1.tv_usec;
		  }
		   printf("op_code: %d----time: %d.%d (second), code: %d\n",op_code,  whole, frac,op_code);
		  }*/
