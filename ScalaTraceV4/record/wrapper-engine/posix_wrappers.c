...
RecordPosixCWrapper (char *name, int is_definition)
{
    ...
    if(strcmp(name, "fopen") == 0){
	    fprintf(posixWrapFile, "\tFILE *fp = (FILE *)malloc(sizeof(FILE));\n");
	    fprintf(posixWrapFile, "\t fp = gwrap_%s( ",name);
    } //added by xiaoqing
    else
    ...
    /* finish */
    if(strcmp(name, "fopen") == 0)
	    fprintf (posixWrapFile, "\treturn fp;\n");
    else
...
RecordGenericPosixOpStart (char *name, int warn_omit)
{
    ...
    for (i = 1; i <= argCounter; i++) {
	    ...
	    else if (strstr(args[i].name, "filestream") != NULL){
	    	fprintf(posixWrapFile, 
			"uop->data.%s.%s = %s;\n",
			nameStr, args[i].name, args[i].name);
	    } //added by xiaoqing
...
void MakePosixCall(char *name) {
  ...
  if((strcmp(name,"fopen") == 0)){
	fprintf(posixWrapFile, "\tFILE *fp = (FILE *)malloc(sizeof(FILE));\n");
	fprintf(posixWrapFile, "\tfp = __real_%s( ",name);
	}//added by xiaoqing	
  else
  ...
  fprintf (posixWrapFile, ");\n");
  if(strcmp(name, "fopen") == 0){
	  fprintf(posixWrapFile, "add_ffiles_handle_entry(record_ptr, fp);\n");
	  fprintf(posixWrapFile, "addScalarValue(event, POSIX_FD, ffiles_handle_to_index(record_ptr, fp), my_rank);\n");
	  fprintf(posixWrapFile, "long offset = ftell(fp);\n");
	  fprintf(posixWrapFile, "addScalarValue(event, OFFSET, offset, my_rank);\n");
	  fprintf(posixWrapFile, "if(Issystem_open(filename))\n");
	  fprintf(posixWrapFile, "   add_fexclusions(fp);\n");
  }
}
...
RecordPosixWrapper (char *name)
{
    ...
    if(strcmp(name, "fopen") == 0) //added by xiaoqing
	fprintf(posixWrapFile, "\nstatic FILE * \ngwrap_%s( ",name);
    else
    	fprintf (posixWrapFile, "\nstatic int\ngwrap_%s( ", name);
    PrintAllPosixGenericArgs ();
