...
//This is for io-tracing in lower level, ignore the io of operating system 
vector<FILE*> fexclusions;
vector<int> fdexclusions;
...
static void init_fds_handles(HandleMapper<int>& fds) {
    if(fds.num_builtins()) return;
    fds.add_builtin(0);
    fds.add_builtin(1);
    fds.add_builtin(2);
}

/*static void init_ffiles_handles(HandleMapper<FILE>& ffiles) {
    if(ffiles.num_builtins()) return;
      ffiles.add_builtin((FILE*)0);
}
*/
...
struct ptr_handler {
  ...
  HandleMapper<int>          fds;
  //HandleMapper<FILE>         ffiles;

  ptr_handler(int bufsize = PTR_HANDLER_SZ) 
    : types(bufsize), comms(bufsize), groups(bufsize), ops(bufsize), files(bufsize), fds(bufsize) { 
    ...
    init_fds_handles(fds);
    fds.clear_handle_buffer();
    /*init_ffiles_handles(ffiles);
    ffiles.clear_handle_buffer();*/
  } 
};
...
/*  int ret = -1;
  ret = ptr->types.handle_to_index(type);
  if(ret == -1){
    ptr->types.add_builtin(type);
    ret = ptr->types.handle_to_index(type);
  }
  return ret;
  */
...
/*  int ret = -1;
  ret = ptr->comms.handle_to_index(comm);
  if(ret == -1){
    ptr->comms.add_builtin(comm);
    ret = ptr->comms.handle_to_index(comm);
  }
  return ret;
  */
...
int fds_handle_to_index(ptr_handler_t ptr, int fds) {
    return ptr->fds.handle_to_index(fds);
}
int index_to_fds_handle(ptr_handler_t ptr, int index) {
    return ptr->fds.index_to_handle(index);
}

void add_fds_handle_entry(ptr_handler_t ptr, int fds) {
    ptr->fds.add_handle(fds);
}
void remove_fds_handle_entry(ptr_handler_t ptr, int fds){
    ptr->fds.remove_handle(fds);
}


int ffiles_handle_to_index(ptr_handler_t ptr, FILE *file) {
    int i;
...
FILE *index_to_ffiles_handle(ptr_handler_t ptr, int index) {
    if(ffiles.size()<=index) return NULL;
    return ffiles.at(index);
}

void add_ffiles_handle_entry(ptr_handler_t ptr, FILE *file) {
    if(ffiles.size() == 0)
      ffiles.push_back(NULL);
    ffiles.push_back(file);
}
void remove_ffiles_handle_entry(ptr_handler_t ptr, FILE *file){
    int i; 
    for(i=0; i<ffiles.size(); i++){
        if(ffiles.at(i) == file)
          ffiles.at(i) = NULL;
...
int Issystem_open(const char *filename){
	const char *temp = exclusions[0];
	int i=0;
	while(temp != NULL){
		if(strncasecmp(temp, filename, strlen(temp)) == 0)
			return 1;
		i += 1;
		temp = exclusions[i];	
	}
...
int Issystem_io(int fd){
	if(find(fdexclusions.begin(), fdexclusions.end(), fd)!=fdexclusions.end())
		return 1;
	else
		return 0;
}

void remove_fdexclusions(int fd){
	vector<int>::iterator it = find(fdexclusions.begin(), fdexclusions.end(), fd);
	assert(it != fdexclusions.end());
	fdexclusions.erase(it);	
}

void add_fexclusions(FILE *fp){
	fexclusions.push_back(fp);
}

void add_fdexclusions(int fd){
	fdexclusions.push_back(fd);
}

void remove_fexclusions(FILE *fp){
	vector<FILE *>::iterator it = find(fexclusions.begin(), fexclusions.end(), fp);
	assert(it!=fexclusions.end());
	fexclusions.erase(it);
}

int Issystem_fio(FILE *fp){
	if(find(fexclusions.begin(), fexclusions.end(), fp)!=fexclusions.end())
		return 1;
	else
		return 0;
}
