...
void Loop::onlineInsertLoop(int mem, int iter, Ranklist rl){
	extern int my_rank;
	loop_t *newLoop = new loop_t;
	newLoop->mem = new Param(LOOPMEM);
	newLoop->mem->initWithScalar(mem, my_rank);
	newLoop->mem->setRanklist(rl);
	newLoop->iter = new Param(LOOPITER);
	newLoop->iter->initWithScalar(iter, my_rank);
	newLoop->iter->setRanklist(rl);
	loops.insert(loops.begin(), newLoop);
}
...
void Loop::onlineMergeLoopStack(Loop* other){
	
	vector<loop_t *> *otherLoopStack = other->getLoops();
	loops.clear(); // = otherLoopStack;

	loops.insert(loops.end(), otherLoopStack->begin(), otherLoopStack->end());
}
